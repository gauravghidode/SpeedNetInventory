...
export const getAllActiveConnections = async (req, res) => {
    ...
    const { page, keyword, pageSize } = req.query;
    console.log(keyword, pageSize, page);
    ...
    try {
        const phoneNo = await PhoneNo.find(query).limit(pageSize).skip(skip).populate({
            path: 'newAccount',
        }).populate({
            path: 'vendor',
        });
...
export const addConnection = async (req, res) => {
    try {
        ...
        const {updatedICCID, lastUser, invoiceDay, cplanPrice, cmodemLease, cactivationDate, custody } = req.body;
        const updatedAccountNo = req.body.updatedAccountNo.toString();
        ...
        if (phoneNo.accountStatus == 'active') {
            res.status(200).json({ success: false, message: "Phone already in use" });
            return;
        }
        if (verify && verify.phoneNo !== phoneNo.phoneNo) {
            ...
            if (account.role === 'dealer') {
                await PhoneNo.findByIdAndUpdate(req.params.id, { ICCID: updatedICCID, newAccount: account._id, lastUser: lastUser, accountStatus: 'active', customerFName: account.customerFName, accountNo: account.accountNo, invoiceDay, cplanPrice, cmodemLease, cactivationDate, custody, });
